...
//   });
  ...
  const handleGenerateQRCode = async () => {
    ...
    const linkedinUrl = `https://www.linkedin.com/${inputValue}/`
    ...
    setTimeout(async () => {
        try {
          ...
          setQRCodeImage(uri);
          await saveToHistory(linkedinUrl);
        } catch (error) {
  ...
  const saveToHistory = async (linkedinUrl) => {
    const newEntry = {
      type: "linkedin QRCode",
      data: linkedinUrl,
      url: linkedinUrl,
      ...
      >
        ...
        <Text className="text-white text-lg font-pbold mb-2">
         ...
         <Text className="text-white text-sm font-plight mb-4">
                    Example: michaelbodnar or jackson67
        </Text>
        ...
        >
          <View className="bg-white rounded-lg p-6 w-11/12 items-center shadow-lg">
            ...
            <ViewShot ref={qrRef} options={{ format: "png", result: "data-uri" }}>
                <QRCode
                 value={`https://www.linkedin.com/${inputValue}/`} size={200} />
            </ViewShot>
            </View>
