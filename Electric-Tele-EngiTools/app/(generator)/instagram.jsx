...
//   });
  ...
  const handleGenerateQRCode = async () => {
    ...
    const InstagramUrl = `https://www.instagram.com/${inputValue}/`
    ...
    setTimeout(async () => {
        try {
          ...
          setQRCodeImage(uri);
          await saveToHistory(InstagramUrl);
        } catch (error) {
  ...
  const saveToHistory = async (InstagramUrl) => {
    const newEntry = {
      type: "Instagram QRCode",
      data: InstagramUrl,
      url: InstagramUrl,
        ...
        >
          <View className="bg-white rounded-lg p-6 w-11/12 items-center shadow-lg">
            ...
            <ViewShot ref={qrRef} options={{ format: "png", result: "data-uri" }}>
                <QRCode
                 value={`https://www.instagram.com/${inputValue}/`} size={200} />
            </ViewShot>
            </View>
