...
//   });
  ...
  const handleGenerateQRCode = async () => {
    if (!phoneNumber) {
        return alert("Please enter a phone number to generate an SMS QR Code.");
      }
   
    const data = `Phone: ${phoneNumber}\nMessage: ${message}`;
    setDataToEncode(data);
...
const handleDownloadQRCode = async () => {
  ...
  const handleCopyToClipboard = () => {
    Clipboard.setStringAsync(dataToEncode);
    ...
    <SafeAreaView className="bg-primary flex-1">
      <ScrollView contentContainerStyle={{ flexGrow: 1, justifyContent: "space-evenly", padding: 20 }} showsVerticalScrollIndicator={false}>
        ...
        <View className="items-center mb-8 mt-10">
          ...
          <Text className="text-white text-lg text-center mt-2 font-pbold">Create QR Code for text messages</Text>
          <Image source={icons.sms} style={{ width: 80, height: 80 }} className="mt-4" tintColor="#fff" resizeMode="contain" />
        </View>
        ...
        {/* {phoneNumber || message ? (
            <View className="items-center my-4">
                <QRCode value={`Phone: ${phoneNumber}\nMessage: ${message}`} size={150} />
            </View>
            ) : null} */}
        ...
        >
          ...
          <ViewShot ref={qrRef} options={{ format: "png", result: "data-uri" }}>
            ...
            <View className="flex-row flex-wrap justify-between w-full gap-4">
              ...
              >
                <Image source={icons.printing} className="w-5 h-5 mr-2" tintColor="#fff" />
                <Text className="text-white font-pregular">Copy</Text>
              </TouchableOpacity>
