...
//   });
  ...
  const handleGenerateQRCode = async () => {
    ...
    const youtubeUrl = `https://www.youtube.com/${inputValue}/`
    ...
    setTimeout(async () => {
        try {
          ...
          setQRCodeImage(uri);
          await saveToHistory(youtubeUrl);
        } catch (error) {
  ...
  const saveToHistory = async (youtubeUrl) => {
    const newEntry = {
      type: "youtube QRCode",
      data: youtubeUrl,
      url: youtubeUrl,
    ...
    <SafeAreaView className="bg-primary flex-1">
      ...
      >
        ...
        {/* Input Field */}
         ...
         Youtube Channel username  </Text>
          <Text className="text-white text-sm font-plight mb-4">
               Example: MrBeast or cristiano
           </Text>
        ...
        >
          <View className="bg-white rounded-lg p-6 w-11/12 items-center shadow-lg">
            ...
            <ViewShot ref={qrRef} options={{ format: "png", result: "data-uri" }}>
                <QRCode
                 value={`https://www.youtube.com/${inputValue}/`} size={200} />
            </ViewShot>
            </View>
