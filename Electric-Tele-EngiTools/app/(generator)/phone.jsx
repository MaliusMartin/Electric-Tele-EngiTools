...
const Phone = () => {
  ...
  const handleGenerateQRCode = async () => {
    ...
    const phoneUrl = `tel:${countryCode}${cleanPhoneNumber}`;  // Add "tel:" protocol for proper QR code scanning
  
    setphoneUrl(phoneUrl);
    setQRCodeModalVisible(true); // Show modal first
    
    // Wait longer for the QR code to render properly
    setTimeout(async () => {
      try {
        if (qrRef.current) {
          const uri = await qrRef.current.capture();
          setQRCodeImage(uri);
          await saveToHistory(phoneUrl);
  ...
  const saveToHistory = async (phoneUrl) => {
    const newEntry = {
      type: "phone QR Code",
      data: phoneUrl,
      url: phoneUrl,
  ...
  const handleDownloadQRCode = async () => {
    ...
    try {
      ...
      if (status !== "granted") {
        return alert("Permission to access media library is required to save the QR code!");
      }
      ...
      const filename = `PhoneQRCode_${Date.now()}.png`;
      ...
      // Save to gallery and create album
  ...
  const handleShareQRCode = async () => {
    ...
    try {
      const base64Data = qrCodeImage.replace(/^data:image\/png;base64,/, "");
      const fileUri = `${FileSystem.cacheDirectory}phoneQR_${Date.now()}.png`;
      await FileSystem.writeAsStringAsync(fileUri, base64Data, {
        encoding: FileSystem.EncodingType.Base64,
      });
    ...
    <SafeAreaView className="flex-1 bg-primary">
      ...
      >
        ...
        <View className="items-center mb-8 mt-10">
          ...
          <Text className="text-white text-lg text-center mt-2 font-pbold">Create QR Code for phone number</Text>
          <Image source={icons.phone} style={{ width: 80, height: 80 }} className="mt-4" tintColor="#fff" resizeMode="contain" />
        </View>
      ...
      >
        ...
        >
          <View className="bg-white rounded-xl p-6 w-11/12 max-w-md items-center shadow-lg">
            ...
            {/* Inside your modal */}
<View className="w-full h-64 mb-4 justify-center items-center bg-white">
  <ViewShot 
    ref={qrRef} 
    options={{ 
      format: "png", 
