...
//   });
  ...
  const handleGenerateQRCode = async () => {
    ...
    const FacebookUrl = `https://www.facebook.com/${inputValue}/`
    ...
    setTimeout(async () => {
        try {
          ...
          setQRCodeImage(uri);
          await saveToHistory(FacebookUrl);
        } catch (error) {
  ...
  const saveToHistory = async (FacebookUrl) => {
    const newEntry = {
      type: "Facebook QRCode",
      data: FacebookUrl,
      url: FacebookUrl,
      ...
      >
        ...
        {/* Input Field */}
         <Text className="text-white text-lg font-pbold mb-2">
          ...
          <Text className="text-white text-sm font-plight mb-4">
               Example: davidbeckham or victoriabeckham
           </Text>
        ...
        >
          <View className="bg-white rounded-lg p-6 w-11/12 items-center shadow-lg">
            ...
            <ViewShot ref={qrRef} options={{ format: "png", result: "data-uri" }}>
                <QRCode
                 value={`https://www.Facebook.com/${inputValue}/`} size={200} />
            </ViewShot>
            </View>
